...
from tkinter import Checkbutton, Button, scrolledtext, ttk
...
def setOutputDeviceFun(Label=None):
    # return setOutputDevice
    def setOutputDevice(event=None):
        deviceName = ''
        deviceIndex = sd.default.device[1]
        # 输出设备

        if (event):
          device = event.widget.get()
          deviceIndex = re.search('(?<=设备编号:)\d+', str(device)).group(0)
        deviceIndex = int(deviceIndex)
        configuration = readConfig()
        configuration['outputDevice'] = deviceIndex
        deviceName = getDeviceList()[deviceIndex]
            # 写入
        configFile = open("./settings.json", "w", encoding="utf8")
        json.dump(configuration, configFile, ensure_ascii=False, indent="\t")
        configFile.close()
        if (Label):
            Label.configure(text=deviceName)
        # 更新活字印刷实例配置
        global HZYS
        HZYS = huoZiYinShua("./settings.json")
    return setOutputDevice
...
def setConfig(option, texts):
    ...
    configFile.close()
    # 更新配置窗口
    optionArray = ["sourceDirectory",
                   "ysddSourceDirectory", "dictFile", "ysddTableFile"]
...
def getDeviceList():
    ...
    for index, device in enumerate(devices):
        devicesNames.insert(index, device['name'] + ' ' + hostapis[device['hostapi']]
                            ['name'] + ' ' + str(device['default_samplerate']) + 'Hz' + "设备编号:"+str(index))
    return devicesNames
# 创建设定窗口
...
def createConfigWindow():
    ...
    text5_1 = Label(configWindow, text="输出设备设置",
                    font=font.Font(family="微软雅黑", size=11))
    devicesNames = getDeviceList()
    text5_2 = Label(configWindow, text=devicesNames[configuration["outputDevice"]], font=font.Font(
        family="微软雅黑", size=11))
    ...
    configButton4 = Button(configWindow, text="选择文件", command=lambda: setConfig("ysddTableFile", texts),
                           height=1, width=8,  font=font.Font(family="微软雅黑", size=11))
    configButton5 = ttk.Combobox(configWindow, width=50,
                                 font=font.Font(family="微软雅黑", size=11))
    configButton5.bind("<<ComboboxSelected>>", setOutputDeviceFun(text5_2))

    configButton5['value'] = devicesNames
    configButton5.current(sd.default.device[1])
    # configButton5['value'] = (1,2,3)
    # 位置
